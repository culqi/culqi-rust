...
impl PlanValidation {
    pub fn create(body: &str) -> Result<(), CustomException> {
        ...
        if let Some(initial_cycles) = json_data["initial_cycles"].as_object() {
            ...
            if let Err(err) = Helpers::validate_initial_cycles(has_initial_charge, currency, count) {
                return Err(err);
            }
    ...
    if let Some(max_amount) = json_data.get("max_amount") {
        if !max_amount.is_i64() {
            return Err(CustomException::new("El filtro 'max_amount' es invalido, debe tener un valor numérico entero."));
        }
    }

    if let Some(min_amount) = json_data.get("min_amount") {
        if !min_amount.is_i64() {
            return Err(CustomException::new("El filtro 'min_amount' es invalido, debe tener un valor numérico entero."));
        }
    }
        Ok(())
    }
